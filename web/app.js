...
const WHAT_DERIVED_YIELD = "Not an external document. No source in our research publishes a Maine per-kW yield — this number is worked backwards from other people's figures, so treat it as unsourced. It is also the single biggest lever in the estimate: every dollar of savings scales with it. A measured figure (an NREL PVWatts run for a Maine location) would replace it.";
...
const OPTIONS = {
  ...
  balcony: {
    ...
    defaults: () => ({
      ...
      specific_yield_kwh_per_kw: A("specific_yield_kwh_per_kw", "Annual production per kW (Maine)", 1200, "kWh/kW/yr", TAGS.DEFAULT_SOURCED,
        S("Derived, not published — no document states this yield", "https://www.nrcm.org/blog/what-to-know-maines-new-plug-in-solar-law/", "UNSOURCED. Worked backwards as $388/yr ÷ $0.27/kWh ÷ 1.2 kW: the linked page gives the Public Advocate's $388/yr savings, not a yield. That division also assumes the $388 came from using ~all of the output on-site — if any was exported, the real yield is higher than 1,200.", WHAT_DERIVED_YIELD),
        "How much electricity one kilowatt of panels actually produces over a year in Maine's real climate — clouds, snow, and winter sun angles included. Multiply by system size to get annual output. Sunnier states run higher; shading, a bad tilt, or a north-facing balcony would drag yours below the default. This is the physics knob of the whole estimate — and the one number here that nobody published: it is worked backwards from the Public Advocate's savings figure, not measured. A vertical railing mount produces meaningfully less than this."),
  ...
  rooftop: {
    ...
    defaults: () => ({
      ...
      specific_yield_kwh_per_kw: A("specific_yield_kwh_per_kw", "Annual production per kW (Maine)", 1200, "kWh/kW/yr", TAGS.DEFAULT_SOURCED,
        S("Unsourced — a conventional round number, no document behind it", null, "UNSOURCED. 1,200 is the rule of thumb used across this model; no source in our research states a Maine yield. (An EnergySage cost page was cited here previously — it gives $/W and payback, but no production figure at all.) Pending an NREL PVWatts run for a Maine location.", WHAT_DERIVED_YIELD),
        "How much electricity one kilowatt of panels produces over a year in Maine's real climate — clouds, snow, and winter sun angles included. A shaded roof, a steep north face, or heavy snow cover pulls it down; an ideal south-facing pitch can beat it slightly. It is also the biggest lever in this estimate and the one number here with no document behind it — a quote from an installer is worth more than our default."),
  ...
  battery: {
    ...
    blurb: "A battery wired into your electrical panel by an electrician, so the house keeps running in an outage. On Maine's ordinary flat rate it saves almost nothing on the bill — you buy it for backup power. Its one money lever is switching to a time-of-use rate (see the plug-in battery for how that earns).",
